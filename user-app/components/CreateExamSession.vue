...
<template>
    <button
        onclick="examCreateLater.showModal()"
        class="btn btn-primary"
    >{{ $t('examCreateLater') }}</button>
    <dialog
        id="examCreateLater"
        ...
        <div class="modal-box">
            ...
            <div class="py-4 grid grid-cols-4 gap-1">
                ...
                <div class="col-span-3"><input
                        ...
                        checked
                    /></div>
                <div class="self-center">{{$t('patient')}}</div>
                <div class="col-span-3">
                    <span class="" v-if="patient">{{ patient.username }} {{ patient.display }}</span> 
                    <button v-if="patient" @click="removePatient">❌</button>
                    <input
                        type="text"
                        class="input input-sm w-full bg-white"
                        @keyup="findUsers"
                        v-if="!patient"
                    />
                    <div class="py-1" v-if="!patient" style="max-height:300px;overflow-y:auto;">
                        <button class="btn btn-xs w-full flex" v-for="user of userFinding" @click="selectPatient(user)">
